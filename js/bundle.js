...
function slider(_ref) {
  ...
  function addNull(numberAll, numberElement, index) {
    if (numberAll.length < 10) {
      numberElement.textContent = `0${index}`;
    } else {
      numberElement.textContent = index;
    }
  }

  function opacityDots(arr, index) {
  ...
  next.addEventListener('click', () => {
    if (offset == replaceElement(width) * (slides.length - 1)) {
      offset = 0;
    } else {
      offset += replaceElement(width);
    }
    ...
    addNull(slides, current, slideIndex);
    opacityDots(dots, slideIndex);
  });
  prev.addEventListener('click', () => {
    if (offset == 0) {
      offset = replaceElement(width) * (slides.length - 1);
    } else {
      offset -= replaceElement(width);
    }
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', e => {
      ...
      slideIndex = slideTo;
      offset = replaceElement(width) * (slideTo - 1);
      slidesField.style.transform = `translateX(-${offset}px)`;
...
window.addEventListener('DOMContentLoaded', function () {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerId), 50000); //вызов модульного окна при открытии страницы, через какое-то время
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2022-04-16');
});
}();
/******/ })()
;
